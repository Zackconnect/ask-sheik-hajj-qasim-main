...
export async function generateAnswer(
  ...
  // Prefer OpenAI for now (more reliable)
  if (openaiKey) {
    try {
      ...
      // Fall through to UMMAH if OpenAI fails
      if (!ummahKey) {
        throw new Error("The answer could not be generated. Please rephrase and try again.");
      }
      console.log("[Ask Sheikh] Falling back to UMMAH API...");
    }
  }

  // Fallback to UMMAH API if available
  if (ummahKey) {
    try {
